 ...
 * 在服务器上运行：cd backend && node seed_redis.js
...
async function seed() {
  ...
  const thresholds = [
    ...
    { id: 'P2024004', pulseMin: 55, pulseMax: 110, tempMin: 36.0, tempMax: 38.5,
      ...
      ecgRules: [
        { name: '室性早搏', condition: 'PVC > 6/min', severity: 'critical' },
      ] },
  ];
  ...
  console.log('  写入 blacklist:token:a1b2c3d4e5...');
  ...
  console.log('  写入 blacklist:token:f6e5d4c3b2...');

  // 验证
  console.log('\n===== 验证 redis-cli KEYS "*" 输出 =====');
